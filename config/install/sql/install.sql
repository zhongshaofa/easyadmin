...
INSERT INTO `ea_system_menu` VALUES ('254', '228', '运维管理', 'fa fa-desktop', '', '', '_self', '0', '1', '', '1617951748', '1617951803', null);
INSERT INTO `ea_system_menu` VALUES ('255', '254', '定时任务', 'fa fa-list', 'system.crontab/index', '', '_self', '0', '1', '', '1617953025', '1617953053', null);
...
-- ----------------------------
-- Table structure for ea_system_crontab
-- ----------------------------
DROP TABLE IF EXISTS `ea_system_crontab`;
CREATE TABLE `ea_system_crontab`  (
  `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  `title` varchar(60) NOT NULL COMMENT '任务标题',
  `type` tinyint(4) NOT NULL DEFAULT 0 COMMENT '任务类型[0请求url,1执行sql,2执行shell]',
  `frequency` varchar(100) NOT NULL COMMENT '任务频率',
  `shell` varchar(150) NOT NULL DEFAULT '' COMMENT '任务脚本',
  `remark` varchar(255) NOT NULL COMMENT '任务备注',
...
) ENGINE = InnoDB AUTO_INCREMENT = 2 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '定时器任务表' ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of ea_system_crontab
-- ----------------------------
INSERT INTO `ea_system_crontab` VALUES (1, '输出 tp 版本', 2, '*/3 * * * * *', 'php think version', '每隔3秒 记录 tp 版本信息', 0, 1, 1617009687, 1617240616);

-- ----------------------------
-- Table structure for ea_system_crontab_flow
-- ----------------------------
DROP TABLE IF EXISTS `ea_system_crontab_flow`;
CREATE TABLE `ea_system_crontab_flow`  (
  `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `create_time`(`create_time`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '定时器任务流水表' ROW_FORMAT = DYNAMIC;